...
class FuelInspectionEnv(gym.Env):
    """
    ...
    def __init__(self, args=None):
        ...
        # Observation:
        # [x, y, z, vx, vy, vz, sun_angle, num_inspected, tx, ty, tz, fuel_remaining_fraction]
    ...
    def render(self, mode="human"):
        ...
        camera_forward = self._safe_unit(self.pos - self.target_pos)
        ax.quiver(
            self.pos[0],
            self.pos[1],
            self.pos[2],
        ...
        ax.set_title(
            ...
            f"fuel={(100.0 * (self.fuel_remaining / self.max_fuel)):.1f}%, "
    ...
    def _mark_inspected_points(self):
        ...
        distance_to_rso = np.linalg.norm(self.target_pos - self.pos)
    ...
    def step(self, action):
        ...
        fuel_used_this_step = float(np.linalg.norm(thrust_cmd))
        self.total_fuel_used += fuel_used_this_step
        self.fuel_remaining = max(0.0, self.fuel_remaining - fuel_used_this_step)
        ...
        # Penalize fuel consumption aswell now
        reward -= self.fuel_penalty_scale * fuel_used_this_step
        ...
        # add another condition for termination on basis of fuel consumption
        elif self.fuel_remaining <= 0.0:
            truncated = True
            self.status = "Out of Fuel"
        ...
        info = {
            ...
            "fuel_remaining_fraction": float(self.fuel_remaining / self.max_fuel),
            "fuel_used_this_step": fuel_used_this_step,
            "total_fuel_used": float(self.total_fuel_used),
            "fuel_remaining": float(self.fuel_remaining),
            "out_of_fuel": self.status == "Out of Fuel",
        }
    ...
    def _get_obs(self):
        ...
        obs[11] = np.float32(self.fuel_remaining / self.max_fuel)
